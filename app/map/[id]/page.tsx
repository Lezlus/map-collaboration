...
export default function CustomMap() {
  ...
  useEffect(() => {
    ...
    const SERVER_URL = `ws://localhost:8080/ws/general?userId=${userId}&username=${username}`
    ...
    // websocket.current = new WebSocket(`ws://localhost:8080/ws/general?userId=${userId}&username=${username}`);
    // websocket.current.onopen = (ev: Event) => {
    //   const id = localStorage.getItem(USER_ID_LOCALSTORAGE_NAME)
    //   if (id) {
    //     const data: WebSocketMapAction = {
    //       type: "MAP ACTION",
    ...
    //   }
    // }
    // Handle all real-time feautes being placed & users connecting
  ...
  }, [mapActionState, websocketFeatureSend]);
